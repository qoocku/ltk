...
")
  ...
  ;;; proc sendevent {s} {puts "(event \"[regsub {"} [regsub {\\} $s {\\\\}] {\"}]\")"}
   ...
   (send-wish "proc sendevent {s x y keycode char width height root_x root_y mouse_button mouse_wheel_delta} {global server; puts $server \"(:event \\\"$s\\\" $x $y $keycode $char $width $height $root_x $root_y $mouse_button $mouse_wheel_delta)\"; flush $server} ")
...
(defun construct-tk-event (properties)
  ...
  (make-event
   ...
   :mouse-wheel-delta (tenth properties)
   ))
...
(defmethod bind ((w widget) event fun &key append exclusive)
  ...
  (let ((name (create-name)))
    ...
    (format-wish "bind  ~a ~a {~:[~;+~]sendevent ~A %x %y %k %K %w %h %X %Y %b %D ~:[~;;break~]}"
...
(defmethod itembind ((canvas canvas) (item integer) event fun)
  ...
  (let ((name (create-name)))
    (add-callback name fun)
    (format-wish "~a bind ~a ~a {sendevent ~A %x %y %k %K %w %h %X %Y %b %D}" (widget-path canvas) item event name))
  canvas)
...
(defmethod tagbind ((canvas canvas) tag event fun &key exclusive)
  ...
  (let ((name (create-name)))
    (add-callback name fun)
    (format-wish "~a bind ~(~a~) ~a {sendevent ~A %x %y %k %K %w %h %X %Y %b %D ~:[~;;break~]}" (widget-path canvas) tag event name exclusive))
  canvas)
...
(defgeneric wm-attributes (widget))
(defmethod wm-attributes ((w widget))
  (format-wish "senddatastrings [wm attributes ~a]" (widget-path w))
  (read-data))

(defgeneric set-wm-attributes (widget &key
                                      ...
                                      #-:unix type
                                      #-:unix zoomed))
(defmethod set-wm-attributes ((w widget) &key
                                         ;; common to all platforms
                                         alpha
                                         fullscreen
                                         topmost
                                         ...
                                         #-:unix type
                                         #-:unix zoomed)
  (let ((pairs `(;; common to all platforms
                 (alpha ,alpha)
                 (fullscreen ,fullscreen)
                 (topmost ,topmost)
                 #+:windows (disabled ,disabled)
                 #+:windows (toolwindow ,toolwindow)
                 #+:windows (transparentcolor ,transparentcolor)
                 #+:darwin (modified ,modified)
                 #+:darwin (notify ,notify)
                 #+:darwin (titlepath ,titlepath)
                 #+:darwin (transparent ,transparent)
                 #-:unix (type ,type)
                 #-:unix (zoomed ,zoomed))))
    (format-wish "senddatastring [wm attributes ~a ~{~{-~(~a~) ~a~} ~}]"
                 (widget-path w) (remove-if #'(lambda (pair) (null (second pair))) pairs))
    (read-data)))
